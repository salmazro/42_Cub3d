...
char	*mlx_int_gaet_line(char *ptr,int *pos,int size)
{
  int	pos2;
  int	pos3;
  int	pos4;
...
int	mlx_int_gaet_col_name(char *str,int size)
{
  int	result;
...
int	mlx_int_gaet_text_rgb(char *name, char *end)
{
  int	i;
  char	buff[64];
...
void	*mlx_int_parse_xpm(mlx_ptr_t *xvar,void *info,int info_size,char *(*f)())
{
  ...
  while (i--)
	...
	RETURN;
      ...
      rgb_col = mlx_int_gaet_text_rgb(tab[j], tab[j+1]);
      /*      
      if ((rgb_col = mlx_int_gaet_text_rgb(tab[j], tab[j+1]))==-1)
	{
      ...
      */
      if (method)
	colors_direct[mlx_int_gaet_col_name(line,cpp)] = rgb_col;
      //	  rgb_col>=0?mlx_get_color_value(xvar, rgb_col):rgb_col;
      else
	{
	  colors[i].name = mlx_int_gaet_col_name(line,cpp);
  ...
  while (i--)
	...
	{
	  col = 0;
	  col_name = mlx_int_gaet_col_name(line+cpp*x,cpp);
	  if (method)
...
void	*mlx_xpm_file_to_image(mlx_ptr_t *xvar,char *file,int *width,int *height)
{
  ...
  if ((img = mlx_int_parse_xpm(xvar,ptr,size,mlx_int_gaet_line)))
    {
      *width = img->width;
      *height = img->height;
    }
